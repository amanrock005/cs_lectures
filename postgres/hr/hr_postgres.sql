-- =====================================================================
-- HR sample schema: PostgreSQL port of Oracle hr_create.sql,
-- hr_populate.sql and hr_code.sql.
-- Run in pgAdmin Query Tool (F5) or: psql -d <db> -f hr_postgres.sql
-- Re-runnable: drops and recreates schema "hr".
-- =====================================================================
...
-- departments.manager_id -> employees (added after the data load; Oracle disabled and re-enabled it)
...
-- ---------------------------------------------------------------- code (hr_code.sql)
-- Only allow DML during office hours (created DISABLED, like the Oracle original)
CREATE OR REPLACE FUNCTION secure_dml() RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  IF to_char(now(), 'HH24:MI') NOT BETWEEN '08:00' AND '18:00'
     OR to_char(now(), 'DY') IN ('SAT', 'SUN') THEN
    RAISE EXCEPTION 'You may only make changes during normal office hours';
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION secure_employees_trg() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  PERFORM secure_dml();
  RETURN NULL;
END;
...
CREATE TRIGGER secure_employees
  ...
  FOR EACH STATEMENT EXECUTE FUNCTION secure_employees_trg();

ALTER TABLE employees DISABLE TRIGGER secure_employees;

-- Record the old job in job_history when job_id / department_id changes
...
CREATE OR REPLACE FUNCTION update_job_history_trg() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER update_job_history
  ...
  FOR EACH ROW EXECUTE FUNCTION update_job_history_trg();
